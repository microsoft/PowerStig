...
using module .\..\..\Rule.SharePoint.SPWebAppGeneralSettings\Convert\SharePoint.SPWebAppGeneralSettingsRule.Convert.psm1
...
class ConvertFactory
{
    static [System.Collections.ArrayList] Rule ([xml.xmlelement] $Rule)
    {
        ...
        switch ($Rule.rule.check.'check-content')
        {
            ...
            {[SharePoint.SPWebAppGeneralSettingsRuleConvert]::Match($PSItem)}
            {
                $null = $ruleTypeList.Add(
                    [SharePoint.SPWebAppGeneralSettingsRuleConvert]::new($Rule).AsRule()
                )
            }
