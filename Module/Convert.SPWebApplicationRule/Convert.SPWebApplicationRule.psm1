...
<#
    .SYNOPSIS
        ...
        SPWebApplicationRule object
    .DESCRIPTION
        The SPWebApplicationRule class is used to extract the SharePoint web
        application settings from the check-content of the xccdf. Once a STIG
        rule is identified as a SharePoint web application rule, it is passed
        to the SPWebApplicationRule class for parsing and validation.

        TODO: need to determine how we're going to organize the various Web Application
              rules so we can properly break them out and process them.
...
#>
Class SPWebApplicationRule : STIG
{
    ...
    <#
        .SYNOPSIS
            Default constructor
        .DESCRIPTION
            Converts a xccdf STIG rule element into a SPWebApplicationRule
        .PARAMETER StigRule
            The STIG rule to convert
    #>
    SPWebApplicationRule ( [xml.xmlelement] $StigRule )
    {
    ...
    {
        return Test-MultipleSPWebApplicationRule -CheckContent ( [STIG]::SplitCheckContent( $CheckContent ) )
    }
    ...
    {
        return ( Split-MultipleSPWebApplicationRule -CheckContent ( [STIG]::SplitCheckContent( $CheckContent ) ) )
    }
