...
$setDynamicClassFileParams = @{
    ClassModuleFileName = 'WebConfigurationPropertyRule.Convert.psm1'
    PowerStigBuildPath  = $script:moduleRoot
    DestinationPath     = (Join-Path -Path $PSScriptRoot -ChildPath '..\.DynamicClassImport\WebConfigurationPropertyRule.Convert.ps1')
}
