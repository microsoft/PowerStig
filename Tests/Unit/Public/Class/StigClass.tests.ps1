#region Header
using module ..\..\..\..\src\public\class\stigClass.psm1
...
#endregion
#region Tests Class
Describe "$ruleClassName Base Class" {
    ...
    Context 'Static Methods' {
        ...
        {    
            It "Should have a method named '$method'" {
                ( [STIG] | Get-Member -Static -Name $method ).Name | Should Be $method
            } 
        }
        # If new methods are added this will catch them so test coverage can be added
        It "Should not have more static methods than are tested" {
            $memberPlanned = $staticMethods + @('Equals', 'new', 'ReferenceEquals')
            $memberActual = ( [STIG] | Get-Member -Static -MemberType Method ).Name
            $compare = Compare-Object -ReferenceObject $memberActual -DifferenceObject $memberPlanned
            $compare.Count | Should Be 0
        }
    }
...
Describe 'SplitCheckContent static method' {
    ...
    [string[]] $splitCheckContent = [STIG]::SplitCheckContent( $CheckContent )
