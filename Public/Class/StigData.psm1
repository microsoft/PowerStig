...
using module .\StigException.psm1
using module .\SkippedRuleType.psm1
using module .\SkippedRule.psm1
using module .\OrganizationalSetting.psm1
using module .\Technology.psm1
using module .\TechnologyRole.psm1
    ...
    [void] MergeOrganizationalSettings ()
    {
        ...
        if ($orgSettingsExists)
        {
            ...
            if ($this.OrganizationalSettings)
            {
                ...
                {
                    $matchingOrgSetting = $this.OrganizationalSettings.Where({$_.StigRuleId -eq $orgSetting.StigRuleId})
            ...
            foreach ( $node in $this.StigXml.DISASTIG.ChildNodes.Name )
            {
                ...
                foreach ( $rule in $this.StigXml.DISASTIG.$node.Rule )
                {
                    ...
                    {
                        $orgSetting = $this.OrganizationalSettings.where({$_.StigRuleId -eq $rule.id})
